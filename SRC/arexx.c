...
void CmdFunc_GetFilePath_Do( struct CmdObj *co )
{
  ULONG len;
    ...
    /*if (BufLength( AGP.RexxResult ) == 0) commenté pour avoir le path complet */
    /* AGP.FObj->OpenFileName peut être vide */
...
{
    if (real > gf->eline)
        return(1); // arrête ici car après la fourchette

    if (real >= gf->sline)
        {
        //if (foldid < gf->id) return(1); // arrête ici car on remonte : erreur...
        if (foldid == gf->id || (foldid > gf->id && (gf->sub || (gf->preid == gf->id && gf->firstsubline))))  // si égal ou si supérieur et option SUB
            {
